...
<div class="card">
    <div class="card-body">
        <h5 class="card-title fw-semibold">Edit Data Profil</h5>

        <form action="<?= base_url("KMS/profil/$userId/update"); ?>" method="post">
            <?= csrf_field(); ?>
            <input type="hidden" name="_method" value="PUT">
            <div class="row">
                <div class="col-12 col-md-6">
                    <div class="my-3">
                        <?php if (auth()->user()->getGroups()[0] == 'admin' || 'developer') { ?>
                        ...
                        <?php } else { ?>
                            <label for="username" class="form-label">Username</label>
                            <input type="text" class="form-control <?php if ($validation->hasError('username')) : ?>is-invalid<?php endif ?>" id="username" name="username" value="<?= $oldInput['username'] ?? $username; ?>" placeholder="username" required disabled>
                            <div class="invalid-feedback">
                                <?= $validation->getError('username'); ?>
                            </div>
                        <?php } ?>
